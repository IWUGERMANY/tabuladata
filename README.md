# tabuladata

### TABULA tables - parameters for energy performance calculation   

The package consists of data frames with codes, parameter values and typological information used by the TABULA method for calculating the energy demand for heating and dometic hot water. 

---

### Method

Information about the method can be found at the website of the European projects TABULA and EPISCOPE: https://episcope.eu/communication/download/: 

- "TABULA_CommonCalculationMethod.pdf": Description of the method 

- "TABULA.xlsm": TABULA master file, basis for the TABULA WebTool displaying residential building typologies for European countries.   

- "tabula-values.xlsx": File extracting the parameter tables from "TABULA.xlsm"

The tables are also used by the TABULA WebTool available for online example calculations as part of the TABULA building typologies: https://webtool.building-typology.eu/#bm  

The TABULA tables were supplemented in the course of the project MOBASY to enable uncertainty assessment and comparison to measured consumption. The data package "tabuladata" is consistent with the file "tabula-values.xlsx" of the Excel package "EnergyProfile-XL-Package.zip" (Download at https://www.iwu.de/forschung/energie/mobasy/).

---

### Usage

```r
library (tabuladata)

```
---

### License

<a rel="license" href="https://creativecommons.org/licenses/by/4.0/"><img alt="Creative Commons License" style="border-width:0" src="https://i.creativecommons.org/l/by/4.0/80x15.png" /></a><br />This work is licensed under a <a rel="license" href="https://creativecommons.org/licenses/by/4.0/">Creative Commons Attribution 4.0 International License</a>.

---

### Data frames

The following tabula tables are available:

- **'info.sheet':**
    Internal sheet control
    Excel sheet name: 'Tab.Info.Sheet'

- **'const.dataformat':**
    Data format codes
    Excel sheet name: 'Tab.Const.DataFormat'

- **'const.statusdataset':**
    Dataset status codes
    Excel sheet name: 'Tab.Const.StatusDataset'

- **'const.typevariant':**
    Type of building variant (refurbishment variant or new build variant)
    Excel sheet name: 'Tab.Const.TypeVariant'

- **'const.typeintake':**
    Types of data intake
    Excel sheet name: 'Tab.Const.TypeIntake'

- **'const.datatype.building':**
    Building data type
    Excel sheet name: 'Tab.Const.DataType.Building'

- **'const.country':**
    Country codes
    Excel sheet name: 'Tab.Const.Country'

- **'const.language':**
    Language codes
    Excel sheet name: 'Tab.Const.Language'

- **'const.buildingsizeclass':**
    Building size classes
    Excel sheet name: 'Tab.Const.BuildingSizeClass'

- **'const.boundarycondtype':**
    Type of boundary condition input
    Excel sheet name: 'Tab.Const.BoundaryCondType'

- **'const.utilisation':**
    Common definitions of utilisation types
    Excel sheet name: 'Tab.Const.Utilisation'

- **'const.rooftype':**
    Roof type codes
    Excel sheet name: 'Tab.Const.RoofType'

- **'const.atticcond':**
    Space heating situation of the attic storey
    Excel sheet name: 'Tab.Const.AtticCond'

- **'const.cellarcond':**
    Space heating situation of the cellar storey
    Excel sheet name: 'Tab.Const.CellarCond'

- **'const.attneighb':**
    Codes for the number of attached neighbour buildings
    Excel sheet name: 'Tab.Const.AttNeighb'

- **'const.complexfootprint':**
    Degree of geometrical complexity of the footprint shape
    Excel sheet name: 'Tab.Const.ComplexFootprint'

- **'const.complexroof':**
    Degree of geometrical complexity of the roof shape
    Excel sheet name: 'Tab.Const.ComplexRoof'

- **'const.orientation':**
    Geographical orientation
    Excel sheet name: 'Tab.Const.Orientation'

- **'const.thermalbridging':**
    Classification of thermal bridging
    Excel sheet name: 'Tab.Const.ThermalBridging'

- **'const.infiltration':**
    Classification of infiltration, dependent on air tightness
    Excel sheet name: 'Tab.Const.Infiltration'

- **'const.constrborder':**
    Type of construction border situation / location of construction element
    Excel sheet name: 'Tab.Const.ConstrBorder'

- **'const.elementtype':**
    Thermal envelope element types
    Excel sheet name: 'Tab.Const.ElementType'

- **'const.measuretype':**
    Type of refurbishment measure / replacement of existing insulation or elements
    Excel sheet name: 'Tab.Const.MeasureType'

- **'const.energycarrier':**
    Codes for the used energy carriers
    Excel sheet name: 'Tab.Const.EnergyCarrier'

- **'const.systype.hg':**
    Heating system: generator types
    Excel sheet name: 'Tab.Const.SysType.HG'

- **'const.systype.hs':**
    Heating system: storage types
    Excel sheet name: 'Tab.Const.SysType.HS'

- **'const.systype.hd':**
    Heating system: distribution and heat emission types
    Excel sheet name: 'Tab.Const.SysType.HD'

- **'const.systype.ha':**
    Heating system: auxiliary energy types
    Excel sheet name: 'Tab.Const.SysType.HA'

- **'const.systype.vent':**
    Heating system: ventilation types
    Excel sheet name: 'Tab.Const.SysType.Vent'

- **'const.systype.wg':**
    Domestic hot water system: generator types
    Excel sheet name: 'Tab.Const.SysType.WG'

- **'const.systype.ws':**
    Domestic hot water system: storage types
    Excel sheet name: 'Tab.Const.SysType.WS'

- **'const.systype.wd':**
    Domestic hot water system: distribution types
    Excel sheet name: 'Tab.Const.SysType.WD'

- **'const.systype.wa':**
    Domestic hot water system: auxiliary energy types
    Excel sheet name: 'Tab.Const.SysType.WA'

- **'const.systype.size':**
    Building types used to distinguish between different system component sizes
    Excel sheet name: 'Tab.Const.SysType.Size'

- **'const.type.calcadapt':**
    Calculation adaptation types
    Excel sheet name: 'Tab.Const.Type.CalcAdapt'

- **'typologyregion':**
    National typology regions
    Excel sheet name: 'Tab.TypologyRegion'

- **'constryearclass':**
    National construction year classes
    Excel sheet name: 'Tab.ConstrYearClass'

- **'additionalpar':**
    National addtitional parameter for classification (regions, special building types etc.)
    Excel sheet name: 'Tab.AdditionalPar'

- **'uncertainty.levels':**
    Uncertainty levels: 5 categories of input data uncertainty; one row for each relevant input variable; for each relevant input variable: explanation of each category; quantification of absolute and/or relative uncertainty
    Excel sheet name: 'Tab.Uncertainty.Levels'

- **'climate':**
    National and regional climate conditions
    Excel sheet name: 'Tab.Climate'

- **'boundarycond':**
    Boundary conditions for the energy balance calculation (optional)
    Excel sheet name: 'Tab.BoundaryCond'

- **'par.envareaestim':**
    Parameters for the envelope area estimation procedure
    Excel sheet name: 'Tab.Par.EnvAreaEstim'

- **'building.constr':**
    National definition of construction elements + U-values
    Excel sheet name: 'Tab.Building.Constr'

- **'building.measure':**
    National definition of insulation measures + thermal resistance
    Excel sheet name: 'Tab.Building.Measure'

- **'u.class.constr':**
    National default U-values differentiated by construction year class and construction type
    Excel sheet name: 'Tab.U.Class.Constr'

- **'insulation.default':**
    National default values of insulation thickness and thermal conductivity of insulation measures, differentiated by implementation period and construction type; to be used if the actual values are not known for a given construction (usage of these default values increases the uncertainty)
    Excel sheet name: 'Tab.Insulation.Default'

- **'measure.f.default':**
    National default values of insulation fraction, differentiated by construction type, implementation period; two cases: (a) typical insulation fraction of refurbished construtions (indicator "Refurbished", used if actual insulation fraction is not known for a construction); (b) ratio of insulated to total construction area for the building stock of the specific construction period (indicator "NA", values used to model an average state for constructions of unknown state) - usage of these default values increases the uncertainty
    Excel sheet name: 'Tab.Measure.f.Default'

- **'u.windowtype.periods':**
    National default U-values of windows, diferentiated by construction year class and type of glazing and frame
    Excel sheet name: 'Tab.U.WindowType.Periods'

- **'u.class.window':**
    U-values of windows (old scheme used for TABULA WebTool (only three time bands)
    Excel sheet name: 'Tab.U.Class.Window'

- **'system.hg':**
    Heating system / heat generation default values
    Excel sheet name: 'Tab.System.HG'

- **'system.hs':**
    Heating system / heat storage default values
    Excel sheet name: 'Tab.System.HS'

- **'system.hd':**
    Heating system / heat distribution default values
    Excel sheet name: 'Tab.System.HD'

- **'system.ha':**
    Heating system / auxiliary energy default values
    Excel sheet name: 'Tab.System.HA'

- **'system.wg':**
    Domestic hot water system / heat generation default values
    Excel sheet name: 'Tab.System.WG'

- **'system.ws':**
    Domestic hot water system / heat storage default values
    Excel sheet name: 'Tab.System.WS'

- **'system.wd':**
    Domestic hot water system / heat distribution default values
    Excel sheet name: 'Tab.System.WD'

- **'system.wa':**
    Domestic hot water system / auxiliary energy default values
    Excel sheet name: 'Tab.System.WA'

- **'system.h':**
    National configurations of heating system
    Excel sheet name: 'Tab.System.H'

- **'system.w':**
    National configurations of domestic hot water systems
    Excel sheet name: 'Tab.System.W'

- **'system.vent':**
    Default values of ventilation systems
    Excel sheet name: 'Tab.System.Vent'

- **'system.pvpanel':**
    Default values for different PV panel types
    Excel sheet name: 'Tab.System.PVPanel'

- **'system.pv':**
    Default values of PV electricity production depending on orientation
    Excel sheet name: 'Tab.System.PV'

- **'system.coverage':**
    Default values of coverages depending on supply/load ratio
    Excel sheet name: 'Tab.System.Coverage'

- **'system.elprod':**
    Assessment of on-site produced electricity
    Excel sheet name: 'Tab.System.ElProd'

- **'system.setecassess':**
    Definition of energy carrier assessment sets
    Excel sheet name: 'Tab.System.SetECAssess'

- **'system.ec':**
    Energy carrier specification and assessment
    Excel sheet name: 'Tab.System.EC'

- **'calcadapt':**
    Definition of calibration factors for adapting the TABULA calculation results to the typical level of metered consumption or to the typical level of national calculation methods (used for respective estimations)
    Excel sheet name: 'Tab.CalcAdapt'


---

### Variables

A description of the variables can be found in the headers of the table "tabula-values.xlsx" in the Excel package "EnergyProfile-XL-Package.zip" (Download at https://www.iwu.de/forschung/energie/mobasy/). 
